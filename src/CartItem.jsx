...
const CartItem = ({ onContinueShopping }) => {
  ...
  const handleContinueShopping = (e) => {
    ...
    if (onContinueShopping) {
      onContinueShopping(); // Call the provided callback for continuing shopping
    }
  };
  ...
  const handleRemove = (item) => {
    dispatch(removeItem(item.name)); // Dispatch the removeItem action to remove the item
  };

  // Calculate total cost based on quantity for an item
  const calculateTotalCost = (item) => {
    return (parseFloat(item.cost.slice(1)) * item.quantity).toFixed(2); // Multiply cost by quantity
  };
